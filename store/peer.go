...
func (s *storage) PutBlock(block *wire.MsgBlock, token string) error {
	...
	if block.Header.PrevBlock.String() == BlockHash {
		...
		// if previousBlock is orphan then make the same height block unorphan and make the previousBlock unorphan
	...
	//After coming here all blocks will be inserted only.
	blockAtHeight := &model.Block{}

	//this line prints this error :
	//2023/06/22 16:02:40 /CATALOG/BitcoinIndexer/indexer2/store/peer.go:243 record not found
...
func (s *storage) orphanBlock(block *model.Block) error {
	...
	//remove all the transactions from the block
	for _, tx := range txs {
		tx.BlockID = 0
		tx.BlockHash = ""
		tx.BlockIndex = 0
